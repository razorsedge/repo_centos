shared_examples_for 'repo_centos::fasttrack' do |ver|
  ...
  it do
    should contain_yumrepo('fasttrack').with({
  ...
  context 'when mirrorlisturl => "http://foo.example.com/centos"' do
    ...
    it { should contain_yumrepo('fasttrack').with_mirrorlist("http://foo.example.com/centos/?release=\$releasever&arch=\$basearch&repo=fasttrack&infra=$infra") }
  end
  ...
  if Gem::Version.new(PUPPET_VERSION) >= Gem::Version.new('3.5.0')
    it { should contain_yumrepo('fasttrack').with_ensure('present') }

    context 'when ensure_fasttrack => "absent"' do
      let(:params) {{ :ensure_fasttrack => "absent" }}
      it { should contain_yumrepo('fasttrack').with_ensure('absent') }
    end
  end
